...
constexpr Square PieceTypeToSquareHandTable[kNumberOfColor][kPieceTypeMax] = {
    {kSquareHand, kBlackHandPawn, kBlackHandLance, kBlackHandKnight,
     kBlackHandSilver, kBlackHandBishop, kBlackHandRook, kBlackHandGold,
     kSquareHand, kBlackHandPawn, kBlackHandLance, kBlackHandKnight,
     kBlackHandSilver, kBlackHandBishop, kBlackHandRook},
    {kSquareHand, kWhiteHandPawn, kWhiteHandLance, kWhiteHandKnight,
     kWhiteHandSilver, kWhiteHandBishop, kWhiteHandRook, kWhiteHandGold,
     kSquareHand, kWhiteHandPawn, kWhiteHandLance, kWhiteHandKnight,
     kWhiteHandSilver, kWhiteHandBishop, kWhiteHandRook}};

void initialize_zobrist() {
  std::mt19937_64 engine(0);
  Zobrist::side = 1;
  Zobrist::exclusion = engine() & ~1;
  for (PieceType piece_type = kOccupied; piece_type < kPieceTypeMax;
       ++piece_type) {
...
inline bool can_promote(Color color, uint32_t from, uint32_t to) {
  if (color == kBlack)
...
void Position::initialize() { initialize_zobrist(); }

void Position::set(const std::string &sfen, Thread *t) {
  char token;
  int sq = 0;
  ...
  while ((ss >> token) && !isspace(token)) {
    ...
    if (piece_letters.find(token) != piece_letters.end()) {
      ...
      if (is_promote) piece = piece + kFlagPromoted;
  ...
  state_->checkers_bb =
      attacks_to(square_king_[side_to_move_], ~side_to_move_, occupied());
  if (state_->checkers_bb.test()) state_->continuous_checks[~side_to_move_] = 1;

  uint8_t list_index = 0;
  for (int i = 1; i <= number_of(hand_[kBlack], kPawn); ++i) {
    kpp_list_[kBlack][list_index] = kFHandPawn + i;
    kpp_list_[kWhite][list_index] = kEHandPawn + i;
    kpp_list_index_[kBlackHandPawn + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kWhite], kPawn); ++i) {
    kpp_list_[kBlack][list_index] = kEHandPawn + i;
    kpp_list_[kWhite][list_index] = kFHandPawn + i;
    kpp_list_index_[kWhiteHandPawn + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kBlack], kLance); ++i) {
    kpp_list_[kBlack][list_index] = kFHandLance + i;
    kpp_list_[kWhite][list_index] = kEHandLance + i;
    kpp_list_index_[kBlackHandLance + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kWhite], kLance); ++i) {
    kpp_list_[kBlack][list_index] = kEHandLance + i;
    kpp_list_[kWhite][list_index] = kFHandLance + i;
    kpp_list_index_[kWhiteHandLance + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kBlack], kKnight); ++i) {
    kpp_list_[kBlack][list_index] = kFHandKnight + i;
    kpp_list_[kWhite][list_index] = kEHandKnight + i;
    kpp_list_index_[kBlackHandKnight + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kWhite], kKnight); ++i) {
    kpp_list_[kBlack][list_index] = kEHandKnight + i;
    kpp_list_[kWhite][list_index] = kFHandKnight + i;
    kpp_list_index_[kWhiteHandKnight + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kBlack], kSilver); ++i) {
    kpp_list_[kBlack][list_index] = kFHandSilver + i;
    kpp_list_[kWhite][list_index] = kEHandSilver + i;
    kpp_list_index_[kBlackHandSilver + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kWhite], kSilver); ++i) {
    kpp_list_[kBlack][list_index] = kEHandSilver + i;
    kpp_list_[kWhite][list_index] = kFHandSilver + i;
    kpp_list_index_[kWhiteHandSilver + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kBlack], kGold); ++i) {
    kpp_list_[kBlack][list_index] = kFHandGold + i;
    kpp_list_[kWhite][list_index] = kEHandGold + i;
    kpp_list_index_[kBlackHandGold + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kWhite], kGold); ++i) {
    kpp_list_[kBlack][list_index] = kEHandGold + i;
    kpp_list_[kWhite][list_index] = kFHandGold + i;
    kpp_list_index_[kWhiteHandGold + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kBlack], kBishop); ++i) {
    kpp_list_[kBlack][list_index] = kFHandBishop + i;
    kpp_list_[kWhite][list_index] = kEHandBishop + i;
    kpp_list_index_[kBlackHandBishop + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kWhite], kBishop); ++i) {
    kpp_list_[kBlack][list_index] = kEHandBishop + i;
    kpp_list_[kWhite][list_index] = kFHandBishop + i;
    kpp_list_index_[kWhiteHandBishop + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kBlack], kRook); ++i) {
    kpp_list_[kBlack][list_index] = kFHandRook + i;
    kpp_list_[kWhite][list_index] = kEHandRook + i;
    kpp_list_index_[kBlackHandRook + i] = list_index;
    ++list_index;
  }

  for (int i = 1; i <= number_of(hand_[kWhite], kRook); ++i) {
    kpp_list_[kBlack][list_index] = kEHandRook + i;
    kpp_list_[kWhite][list_index] = kFHandRook + i;
    kpp_list_index_[kWhiteHandRook + i] = list_index;
    ++list_index;
  }

  for (Square i = k9A; i < kBoardSquare; ++i) {
    if (squares_[i] != kEmpty && squares_[i] != kBlackKing &&
        squares_[i] != kWhiteKing) {
      kpp_list_index_[i] = list_index;
      kpp_list_[kBlack][list_index] = PieceToIndexBlackTable[squares_[i]] + i;
      kpp_list_[kWhite][list_index] =
          PieceToIndexWhiteTable[squares_[i]] + inverse(i);
      ++list_index;
    }
  }
}
...
void Position::do_move(Move m, StateInfo &new_state) {
  do_move(m, new_state, gives_check(m, CheckInfo(*this)));
}

void Position::do_move(Move m, StateInfo &new_state, bool gives_check) {
  ++nodes_searched_;
  ++game_ply_;
  ...
  if (from >= kBoardSquare) {
    ...
    uint8_t list_index =
        kpp_list_index_[PieceTypeToSquareHandTable[us][drop] + hand_num];
    assert(list_index < 38);
    state_->changed_value[kBlack][0] = kpp_list_[kBlack][list_index];
    new_index_value_[kBlack][0] = PieceToIndexBlackTable[squares_[to]] + to;
    kpp_list_[kBlack][list_index] = PieceToIndexBlackTable[squares_[to]] + to;
    state_->changed_value[kWhite][0] = kpp_list_[kWhite][list_index];
    new_index_value_[kWhite][0] =
        PieceToIndexWhiteTable[squares_[to]] + inverse(to);
    kpp_list_[kWhite][list_index] =
        PieceToIndexWhiteTable[squares_[to]] + inverse(to);
    ...
    if (is_promote) {
      ...
      state_->material += (us == kBlack) ? PromotePieceValueTable[piece_move]
      ...
      if (piece_move == kKing) square_king_[us] = static_cast<Square>(to);
    }
    ...
    if (piece_capture != kPieceNone) {
      ...
      state_->changed_value[kBlack][1] = kpp_list_[kBlack][captured_index];
      new_index_value_[kBlack][1] =
          PieceTypeToBlackHandIndexTable[us][piece_capture] + hand_num;
      kpp_list_[kBlack][captured_index] =
          PieceTypeToBlackHandIndexTable[us][piece_capture] + hand_num;
      state_->changed_value[kWhite][1] = kpp_list_[kWhite][captured_index];
      new_index_value_[kWhite][1] =
          PieceTypeToWhiteHandIndexTable[us][piece_capture] + hand_num;
      kpp_list_[kWhite][captured_index] =
          PieceTypeToWhiteHandIndexTable[us][piece_capture] + hand_num;
      kpp_list_index_[PieceTypeToSquareHandTable[us][piece_capture] +
                      hand_num] = captured_index;
    ...
    if (piece_move != kKing) {
      ...
      state_->changed_value[kBlack][0] = kpp_list_[kBlack][kpp_index];
      new_index_value_[kBlack][0] = PieceToIndexBlackTable[squares_[to]] + to;
      kpp_list_[kBlack][kpp_index] = PieceToIndexBlackTable[squares_[to]] + to;
      state_->changed_value[kWhite][0] = kpp_list_[kWhite][kpp_index];
      new_index_value_[kWhite][0] =
          PieceToIndexWhiteTable[squares_[to]] + inverse(to);
      kpp_list_[kWhite][kpp_index] =
          PieceToIndexWhiteTable[squares_[to]] + inverse(to);
...
void Position::do_null_move(StateInfo &new_state) {
...
void Position::undo_move(Move move) {
  ...
  if (from >= kBoardSquare) {
    ...
    kpp_list_index_[PieceTypeToSquareHandTable[side_to_move_][drop] +
                    number_of(hand_[side_to_move_], drop)] =
        state_->list_index_move;
    kpp_list_[kBlack][state_->list_index_move] =
        state_->changed_value[kBlack][0];
    kpp_list_[kWhite][state_->list_index_move] =
        state_->changed_value[kWhite][0];
  } else {
    ...
    if (piece_capture != kPieceNone) {
      ...
      kpp_list_[kBlack][state_->list_index_capture] =
          state_->changed_value[kBlack][1];
      kpp_list_[kWhite][state_->list_index_capture] =
          state_->changed_value[kWhite][1];
    } else {
      squares_[to] = kEmpty;
    }
  }
...
bool Position::gives_mate_by_drop_pawn(Square sq) const {
  Color color = side_to_move_;
  // 歩の打った場所で王手がかかるか
  if (color == kBlack) {
    if (squares_[sq - 9] != kWhiteKing) return false;
  } else {
    if (squares_[sq + 9] != kBlackKing) return false;
  }
  ...
  occupy.xor_bit(sq);
  BitBoard movable =
      KingAttacksTable[square_king_[enemy]] & ~piece_board_[enemy][kOccupied];
  do {
    Square to = movable.pop_bit();
    // 敵の王が動けるならばその時点で打ち歩づめではない
    if (!is_attacked(to, enemy, occupy)) return false;
  } while (movable.test());
  ...
  sum.and_or((piece_board_[enemy][kHorse] | piece_board_[enemy][kDragon]),
             KingAttacksTable[sq]);
  BitBoard pinned = pinned_pieces(enemy);
  while (sum.test()) {
    Square from = sum.pop_bit();
    // 空き王手になるならその駒は動かせない
    if (!is_king_discover(from, sq, enemy, pinned)) return false;
  }

  return true;
}

inline bool Position::is_pawn_exist(Square sq, Color color) const {
  return (piece_board_[color][kPawn] & FileMaskTable[FilePositionTable[sq]])
      .test();
}

BitBoard Position::check_blockers(Color c, Color king_color,
                                  const BitBoard &occupied) const {
  BitBoard result;
  result.init();
  Square king_square = square_king_[king_color];
  BitBoard pinners = piece_board_[~king_color][kLance] &
                     LanceAttacksTable[king_color][king_square][0];
...
bool Position::validate() const {
  ...
  if (((piece_board_[kBlack][kPawn] | piece_board_[kBlack][kLance] |
  ...
  if (((piece_board_[kWhite][kPawn] | piece_board_[kWhite][kLance] |
...
void Position::put_piece(Piece piece, Square sq) {
  ...
  if (piece == kBlackKing) square_king_[kBlack] = sq;

  if (piece == kWhiteKing) square_king_[kWhite] = sq;
}
...
int Position::compute_material() const {
  ...
  num = static_cast<int>(piece_board_[kBlack][kPawn].popcount()) +
        number_of(hand_[kBlack], kPawn);
  num -= static_cast<int>(piece_board_[kWhite][kPawn].popcount()) +
         number_of(hand_[kWhite], kPawn);
  material += num * PieceValueTable[kPawn];

  num = static_cast<int>(piece_board_[kBlack][kLance].popcount()) +
        number_of(hand_[kBlack], kLance);
  num -= static_cast<int>(piece_board_[kWhite][kLance].popcount()) +
         number_of(hand_[kWhite], kLance);
  material += num * PieceValueTable[kLance];

  num = static_cast<int>(piece_board_[kBlack][kKnight].popcount()) +
        number_of(hand_[kBlack], kKnight);
  num -= static_cast<int>(piece_board_[kWhite][kKnight].popcount()) +
         number_of(hand_[kWhite], kKnight);
  material += num * PieceValueTable[kKnight];

  num = static_cast<int>(piece_board_[kBlack][kSilver].popcount()) +
        number_of(hand_[kBlack], kSilver);
  num -= static_cast<int>(piece_board_[kWhite][kSilver].popcount()) +
         number_of(hand_[kWhite], kSilver);
  material += num * PieceValueTable[kSilver];

  num = static_cast<int>(piece_board_[kBlack][kGold].popcount()) +
        number_of(hand_[kBlack], kGold);
  num -= static_cast<int>(piece_board_[kWhite][kGold].popcount()) +
         number_of(hand_[kWhite], kGold);
  material += num * PieceValueTable[kGold];

  num = static_cast<int>(piece_board_[kBlack][kBishop].popcount()) +
        number_of(hand_[kBlack], kBishop);
  num -= static_cast<int>(piece_board_[kWhite][kBishop].popcount()) +
         number_of(hand_[kWhite], kBishop);
  material += num * PieceValueTable[kBishop];

  num = static_cast<int>(piece_board_[kBlack][kRook].popcount()) +
        number_of(hand_[kBlack], kRook);
  num -= static_cast<int>(piece_board_[kWhite][kRook].popcount()) +
         number_of(hand_[kWhite], kRook);
...
FORCE_INLINE PieceType min_attacker(const Position *pos, Square to,
                                    Color defend_side,
                                    const BitBoard &defenders,
                                    BitBoard &attackers, BitBoard &occupied) {
  const static PieceType min_table[] = {
      kPawn,   kLance,          kKnight,         kPromotedPawn, kPromotedLance,
      kSilver, kPromotedKnight, kPromotedSilver, kGold,         kBishop,
      kRook,   kHorse,          kDragon,         kKing};
  BitBoard b = defenders & pos->pieces(min_table[piece_type], defend_side);
  if (!b.test())
    return min_attacker<piece_type + 1>(pos, to, defend_side, defenders,
                                        attackers, occupied);
  ...
  switch (DirectionTable[to][sq]) {
    case kDirFile:
      ...
      attacks.and_or(pos->pieces(kLance, defend_side),
                     LanceAttacksTable[~defend_side][to][0]);
      attacks &=
          (rook_attack(occupied, to) & FileMaskTable[FilePositionTable[to]]);
      attackers = attackers | attacks;
      break;
    case kDirRank:
      attacks = pos->rook_dragon(defend_side);
      attacks &=
          (rook_attack(occupied, to) & RankMaskTable[RankPositionTable[to]]);
      attackers = attackers | attacks;
      break;
    case kDirLeft45:
      attacks = pos->bishop_horse(defend_side);
      attacks &= (bishop_attack(occupied, to) &
                  Left45MaskTable[Left45MaskIndexTable[to]]);
      attackers = attackers | attacks;
      break;
    case kDirRight45:
      attacks = pos->bishop_horse(defend_side);
      attacks &= (bishop_attack(occupied, to) &
                  Right45MaskTable[Right45MaskIndexTable[to]]);
      attackers = attackers | attacks;
      break;
    default:
      break;
...
template <>
FORCE_INLINE PieceType min_attacker<13>(const Position *pos, Square to,
                                        Color defend_side,
                                        const BitBoard &defenders,
                                        BitBoard &attackers,
                                        BitBoard &occupied) {
...
bool Position::see_ge(Move m, Value v, Color c) const {
  Square to = move_to(m);
  Square from = move_from(m);
  ...
  if (from < kBoardSquare) {
    ...
    if (next_victim == kKing) return true;
...
#endif
    if (!side_to_move_attackers.test()) return relative_side_to_move;

    next_victim = min_attacker<0>(this, to, side_to_move,
                                  side_to_move_attackers, attackers, occupied);

    if (next_victim == kKing)
      return relative_side_to_move ==
             (attackers & piece_board_[~side_to_move][kOccupied]).test();

    balance += relative_side_to_move ? ExchangePieceValueTable[next_victim]
                                     : -ExchangePieceValueTable[next_victim];

    relative_side_to_move = !relative_side_to_move;
    if (relative_side_to_move == (balance >= v)) return relative_side_to_move;
...
uint64_t Position::exclusion_key() const {
  return (state_->board_key + state_->hand_key) ^ Zobrist::exclusion;
}

Repetition Position::in_repetition() const {
...
std::ostream &operator<<(std::ostream &os, const Position &pos) {
  const char *print_name[] = {"  ", "P ", "L ", "N ", "S ", "B ", "R ", "G ",
                              "K ", "P+", "L+", "N+", "S+", "H ", "D ", "",
                              "  ", "p ", "l ", "n ", "s ", "b ", "r ", "g ",
                              "k ", "p+", "l+", "n+", "s+", "h ", "d ", ""};
...
void Position::print() const { std::cout << *this << std::endl; }

bool Position::is_decralation_win() const {
  Color us = side_to_move_;
  // 宣言する者の玉が入玉している
  if (us == kBlack) {
    if (square_king_[kBlack] > k1C) return false;
  } else {
    if (square_king_[kWhite] < k9G) return false;
  }

  // 宣言する者の敵陣にいる駒は、玉を除いて10枚以上である
  if (((piece_board_[us][kOccupied] ^ piece_board_[us][kKing]) &
       PromotableMaskTable[us])
          .popcount() < 10)
    return false;
  ...
  BitBoard large_pieces = piece_board_[us][kBishop] | piece_board_[us][kRook] |
                          piece_board_[us][kDragon] | piece_board_[us][kHorse];
  BitBoard small_pieces =
      (piece_board_[us][kOccupied] ^ piece_board_[us][kKing]);
  small_pieces.not_and(large_pieces);
  large_pieces &= PromotableMaskTable[us];
  small_pieces &= PromotableMaskTable[us];

  int large_score = static_cast<int>(large_pieces.popcount()) +
                    number_of(hand_[us], kBishop) + number_of(hand_[us], kRook);
  int small_score = static_cast<int>(small_pieces.popcount()) +
                    number_of(hand_[us], kPawn) + number_of(hand_[us], kLance) +
                    number_of(hand_[us], kKnight) +
                    number_of(hand_[us], kSilver) + number_of(hand_[us], kGold);
