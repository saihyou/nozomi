...
int16_t g_score_from_to[kNumberOfBoardHand][kBoardSquare][kNumberOfColor]
                       [Eval::kScoreEnd];
int16_t g_score_piece_to[kPieceTypeMax][kBoardSquare][kNumberOfColor]
                        [Eval::kScoreEnd];
int16_t g_score_capture[kPieceTypeMax][kPieceTypeMax][kNumberOfColor]
                       [Eval::kScoreEnd];
...
Value evaluate(const Position &pos, const CheckInfo &ci, Move move) {
  ...
  if (from >= kBoardSquare) {
    after_type = move_type = TypeOf(from);
  } else {
    move_type = move_piece_type(move);
    after_type = move_type;
    if (move_is_promote(move)) after_type = after_type + kFlagPromoted;
  ...
  for (int i = 0; i < Eval::kListNum; ++i) {
    __m128i tmp = _mm_set_epi32(
        from_to[list_black[i]], piece_to[list_black[i]],
        score_capture[list_black[i]], score_check[list_black[i]]);
    total = _mm_add_epi32(total, tmp);
  }
  {
    __m128i tmp = _mm_set_epi32(
        from_to[black_king_index], piece_to[black_king_index],
        score_capture[black_king_index], score_check[black_king_index]);
    total = _mm_add_epi32(total, tmp);
    tmp = _mm_set_epi32(
        from_to[white_king_index], piece_to[white_king_index],
        score_capture[white_king_index], score_check[white_king_index]);
    total = _mm_add_epi32(total, tmp);
  }
...
bool init() {
  ...
  ifs.read(reinterpret_cast<char *>(g_score_piece_to),
           sizeof(g_score_piece_to));
...
struct LearningParamArray {
  LearningParam from_to[kNumberOfBoardHand][kBoardSquare][kNumberOfColor]
                       [Eval::kScoreEnd];
  LearningParam piece_to[kPieceTypeMax][kBoardSquare][kNumberOfColor]
                        [Eval::kScoreEnd];
  LearningParam capture[kPieceTypeMax][kPieceTypeMax][kNumberOfColor]
                       [Eval::kScoreEnd];
  LearningParam check[2][kNumberOfColor][Eval::kScoreEnd];
};

int get_move_ranking(const Position &pos, Move move) {
  CheckInfo ci(pos);
  ...
  for (auto &m : legal_moves) {
    if (m.move == move) return rank;
    ++rank;
  }
  return -1;
}

void load_book(const std::string &file_name,
...
void read(const std::string &book_file) {
  std::vector<std::vector<std::string>> book;
  ...
  for (int i = 0; i < 11; ++i)
    std::cout << "     " << i << " : " << counter[i] << " ("
              << (double)counter[i] / (double)total * 100.0 << " %)"
              << std::endl;
...
struct LearnEval {
  float score_from_to[kNumberOfBoardHand][kBoardSquare][kNumberOfColor]
                     [Eval::kScoreEnd];
  float score_piece_to[kPieceTypeMax][kBoardSquare][kNumberOfColor]
                      [Eval::kScoreEnd];
  float score_capture[kPieceTypeMax][kPieceTypeMax][kNumberOfColor]
                     [Eval::kScoreEnd];
...
LearningParamArray *g_learning_param_array;

void to_normal_value(const LearnEval &eval) {
  for (int f = 0; f < kNumberOfBoardHand; ++f) {
    for (int t = 0; t < kBoardSquare; ++t) {
      for (int p = 0; p < Eval::kFEEnd; ++p) {
        g_score_from_to[f][t][0][p] =
            static_cast<int16_t>(eval.score_from_to[f][t][0][p] * 1024.0);
        g_score_from_to[f][t][1][p] =
            static_cast<int16_t>(eval.score_from_to[f][t][1][p] * 1024.0);
      }
    }
  }
  ...
  for (int a = 0; a < kPieceTypeMax; ++a) {
    for (int t = 0; t < kBoardSquare; ++t) {
      for (int p = 0; p < Eval::kFEEnd; ++p) {
        g_score_piece_to[a][t][0][p] =
            static_cast<int16_t>(eval.score_piece_to[a][t][0][p] * 1024.0);
        g_score_piece_to[a][t][1][p] =
            static_cast<int16_t>(eval.score_piece_to[a][t][1][p] * 1024.0);
      }
    }
  }
  ...
  for (int m = 0; m < kPieceTypeMax; ++m) {
    for (int t = 0; t < kPieceTypeMax; ++t) {
      for (int p = 0; p < Eval::kFEEnd; ++p) {
        g_score_capture[m][t][0][p] =
            static_cast<int16_t>(eval.score_capture[m][t][0][p] * 1024.0);
        g_score_capture[m][t][1][p] =
            static_cast<int16_t>(eval.score_capture[m][t][1][p] * 1024.0);
      }
    }
  }
  ...
  for (int p = 0; p < Eval::kFEEnd; ++p) {
    g_score_check[0][0][p] =
        static_cast<int16_t>(eval.score_check[0][0][p] * 1024.0);
    g_score_check[0][1][p] =
        static_cast<int16_t>(eval.score_check[0][1][p] * 1024.0);
    g_score_check[1][0][p] =
        static_cast<int16_t>(eval.score_check[1][0][p] * 1024.0);
    g_score_check[1][1][p] =
        static_cast<int16_t>(eval.score_check[1][1][p] * 1024.0);
  }
}

void to_floating_value(LearnEval &eval) {
  for (int f = 0; f < kNumberOfBoardHand; ++f) {
    for (int t = 0; t < kBoardSquare; ++t) {
      for (int p = 0; p < Eval::kFEEnd; ++p) {
        eval.score_from_to[f][t][0][p] =
            static_cast<float>(g_score_from_to[f][t][0][p]) / 1024.0f;
        eval.score_from_to[f][t][1][p] =
            static_cast<float>(g_score_from_to[f][t][1][p]) / 1024.0f;
      }
    }
  }
  ...
  for (int a = 0; a < kPieceTypeMax; ++a) {
    for (int t = 0; t < kBoardSquare; ++t) {
      for (int p = 0; p < Eval::kFEEnd; ++p) {
        eval.score_piece_to[a][t][0][p] =
            static_cast<float>(g_score_piece_to[a][t][0][p]) / 1024.0f;
        eval.score_piece_to[a][t][1][p] =
            static_cast<float>(g_score_piece_to[a][t][1][p]) / 1024.0f;
      }
    }
  }
  ...
  for (int m = 0; m < kPieceTypeMax; ++m) {
    for (int t = 0; t < kPieceTypeMax; ++t) {
      for (int p = 0; p < Eval::kFEEnd; ++p) {
        eval.score_capture[m][t][0][p] =
            static_cast<float>(g_score_capture[m][t][0][p]) / 1024.0f;
        eval.score_capture[m][t][1][p] =
            static_cast<float>(g_score_capture[m][t][1][p]) / 1024.0f;
      }
    }
  }

  for (int p = 0; p < Eval::kFEEnd; ++p) {
    eval.score_check[0][0][p] =
        static_cast<float>(g_score_check[0][0][p]) / 1024.0f;
    eval.score_check[0][1][p] =
        static_cast<float>(g_score_check[0][1][p]) / 1024.0f;
    eval.score_check[1][0][p] =
        static_cast<float>(g_score_check[1][0][p]) / 1024.0f;
    eval.score_check[1][1][p] =
        static_cast<float>(g_score_check[1][1][p]) / 1024.0f;
  }
}

void add_atomic_float(std::atomic<float> &target, float param) {
...
struct Gradient {
  std::atomic<float> score_from_to[kNumberOfBoardHand][kBoardSquare]
                                  [kNumberOfColor][Eval::kScoreEnd];
  std::atomic<float> score_piece_to[kPieceTypeMax][kBoardSquare][kNumberOfColor]
                                   [Eval::kScoreEnd];
  std::atomic<float> score_capture[kPieceTypeMax][kPieceTypeMax][kNumberOfColor]
                                  [Eval::kScoreEnd];
  std::atomic<float> score_check[2][kNumberOfColor][Eval::kScoreEnd];
  std::atomic<int> count_from_to[kNumberOfBoardHand][kBoardSquare]
                                [kNumberOfColor][Eval::kScoreEnd];
  std::atomic<int> count_piece_to[kPieceTypeMax][kBoardSquare][kNumberOfColor]
                                 [Eval::kScoreEnd];
  std::atomic<int> count_capture[kPieceTypeMax][kPieceTypeMax][kNumberOfColor]
                                [Eval::kScoreEnd];
...
void Gradient::increment(const Position &pos, const CheckInfo &ci, Move m,
  ...
  PieceType after_type = kPieceNone;
  if (from >= kBoardSquare) {
    move_type = TypeOf(from);
  } else {
    after_type = move_type = move_piece_type(m);
    if (move_is_promote(m)) after_type = after_type + kFlagPromoted;
  ...
  for (int i = 0; i < Eval::kListNum; ++i) {
    ...
    add_atomic_float(score_piece_to[after_type][to][color][list_black[i]],
                     delta);
    add_atomic_float(score_capture[move_type][capture][color][list_black[i]],
                     delta);
  ...
  add_atomic_float(score_piece_to[after_type][to][color][black_king_index],
                   delta);
  add_atomic_float(score_capture[move_type][capture][color][black_king_index],
                   delta);
  ...
  add_atomic_float(score_piece_to[after_type][to][color][white_king_index],
                   delta);
  add_atomic_float(score_capture[move_type][capture][color][white_king_index],
                   delta);
...
void load_param() {
  ...
  if (ifs) {
    ifs.read(reinterpret_cast<char *>(g_score_from_to),
             sizeof(g_score_from_to));
    ifs.read(reinterpret_cast<char *>(g_score_piece_to),
             sizeof(g_score_piece_to));
    ifs.read(reinterpret_cast<char *>(g_score_capture),
             sizeof(g_score_capture));
    ifs.read(reinterpret_cast<char *>(g_score_check), sizeof(g_score_check));
    ifs.close();
  }
...
void save_param() {
  ...
  fs.write(reinterpret_cast<char *>(g_score_piece_to),
           sizeof(g_score_piece_to));
...
size_t read_file(std::ifstream &ifs, std::vector<KifuData> &position_list,
...
double compute_gradient(std::vector<KifuData> &position_list,
                        std::vector<Position> &positions, Gradient *gradient) {
  double diff = 0;
#ifdef _OPENMP
...
void update(float grad, float *param, LearningParam *p, int count,
  ...
  if (count > 0 && (grad > 0.0 || grad < 0.0))
    g = (grad / static_cast<double>(count));
  else
    return;

  double m_t = beta_1 * p->m + (1 - beta_1) * g;
  double v_t = beta_2 * p->v + (1 - beta_2) * g * g;
  double m_hat = m_t / (1 - beta_1);
  double v_hat = v_t / (1 - beta_2);
  double w = learning_rate * m_hat / (std::sqrt(v_hat) + e);
  *param += static_cast<float>(w);
  p->m = static_cast<float>(m_t);
  p->v = static_cast<float>(v_t);
  *param = *param / (1 + 0.01 * learning_rate);
#else
  ...
  if (count > 0 && (grad > 0.0 || grad < 0.0))
    ...
    return;
  float w = static_cast<float>(g * learning_rate + 0.9 * p->w);
  *param += w;
  p->w = w;
  *param = static_cast<float>(*param / (1 + l));
#endif
}

void add_param(Gradient *param, double learning_rate) {
#ifdef _OPENMP
#pragma omp parallel
#endif
  {
    ...
    for (int f = 0; f < kNumberOfBoardHand; ++f) {
      for (int t = 0; t < kBoardSquare; ++t) {
        for (int p = 0; p < Eval::kFEEnd; ++p) {
          ...
          update(grad, &g_floating_eval->score_from_to[f][t][0][p],
                 &g_learning_param_array->from_to[f][t][0][p], count,
                 learning_rate);
          grad = param->score_from_to[f][t][1][p].load();
          count = param->count_from_to[f][t][1][p].load();
          update(grad, &g_floating_eval->score_from_to[f][t][1][p],
                 &g_learning_param_array->from_to[f][t][1][p], count,
                 learning_rate);
        }
      }
    }
    ...
    for (int a = 0; a < kPieceTypeMax; ++a) {
      for (int t = 0; t < kBoardSquare; ++t) {
        for (int p = 0; p < Eval::kFEEnd; ++p) {
          ...
          update(grad, &g_floating_eval->score_piece_to[a][t][0][p],
                 &g_learning_param_array->piece_to[a][t][0][p], count,
                 learning_rate);
          grad = param->score_piece_to[a][t][1][p].load();
          count = param->count_piece_to[a][t][1][p].load();
          update(grad, &g_floating_eval->score_piece_to[a][t][1][p],
                 &g_learning_param_array->piece_to[a][t][1][p], count,
                 learning_rate);
        }
      }
    }
    ...
    for (int m = 0; m < kPieceTypeMax; ++m) {
      for (int t = 0; t < kPieceTypeMax; ++t) {
        for (int p = 0; p < Eval::kFEEnd; ++p) {
          ...
          update(grad, &g_floating_eval->score_capture[m][t][0][p],
                 &g_learning_param_array->capture[m][t][0][p], count,
                 learning_rate);
          grad = param->score_capture[m][t][1][p].load();
          count = param->count_capture[m][t][1][p].load();
          update(grad, &g_floating_eval->score_capture[m][t][1][p],
                 &g_learning_param_array->capture[m][t][1][p], count,
                 learning_rate);
        }
      }
    }
...
#pragma omp for
    ...
    for (int p = 0; p < Eval::kFEEnd; ++p) {
      ...
      update(grad, &g_floating_eval->score_check[0][0][p],
             &g_learning_param_array->check[0][0][p], count,
             learning_rate);
      grad = param->score_check[0][1][p].load();
      count = param->count_check[0][1][p].load();
      update(grad, &g_floating_eval->score_check[0][1][p],
             &g_learning_param_array->check[0][1][p], count,
             learning_rate);
      grad = param->score_check[1][0][p].load();
      count = param->count_check[1][0][p].load();
      update(grad, &g_floating_eval->score_check[1][0][p],
             &g_learning_param_array->check[1][0][p], count,
             learning_rate);
      grad = param->score_check[1][1][p].load();
      count = param->count_check[1][1][p].load();
      update(grad, &g_floating_eval->score_check[1][1][p],
             &g_learning_param_array->check[1][1][p], count,
             learning_rate);
    }
  }
}

void update_param(const std::string &record_file_name,
                  std::vector<Position> &positions, int num_threads,
                  double learning_rate) {
  load_param();
  double win_diff = 0;
...
void reinforce(std::istringstream &is) {
  ...
  g_learning_param_array = static_cast<LearningParamArray *>(
      std::malloc(sizeof(LearningParamArray)));
  if (g_score_check == nullptr) {
    free(g_floating_eval);
    return;
  }
